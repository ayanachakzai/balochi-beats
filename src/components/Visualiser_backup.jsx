  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      if (analyserRef.current && dataArrayRef.current) {
        ...
        // ── Beat detection ──
        const bassSlice = Array.from(freqData.slice(0, 10))
        const bassEnergy = bassSlice.reduce((a, b) => a + b, 0) / 10 / 255
        rollingRef.current = rollingRef.current * 0.93 + bassEnergy * 0.07
        const threshold = Math.max(rollingRef.current * 1.2, 0.08)
        const isBeatCandidate = bassEnergy > threshold && beatCoolRef.current <= 0

        if (isBeatCandidate) {
          isBeat = true
          beatCoolRef.current = 14
        }
      ...
      if (isBeat && currentMode === 'beats') {
        for (let i = 0; i < 40; i++) {
          ...
          const pMtl = new THREE.MeshBasicMaterial({
            color:       0xe8a020,
            transparent: true,
            opacity:     1.0,
          })
          const p = new THREE.Mesh(pGeo, pMtl)
          p.position.copy(sphere.position)
          p.userData.vx    = (Math.random() - 0.5) * 0.4
          p.userData.vy    = (Math.random() - 0.5) * 0.4
          p.userData.vz    = (Math.random() - 0.5) * 0.4
          p.userData.decay = 1 / 30
          scene.add(p)
